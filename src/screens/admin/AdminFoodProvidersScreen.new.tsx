...
const AdminFoodProvidersScreen: React.FC = () => {
  ...
  const handleProviderAction = async (providerId: string, action: 'approve' | 'reject' | 'activate' | 'deactivate') => {
    try {
      ...
      switch (action) {
        case 'approve':
          endpoint = `/food-providers/${providerId}/approve`;
          message = 'Food provider approved successfully';
          break;
        case 'reject':
          endpoint = `/food-providers/${providerId}/reject`;
          message = 'Food provider rejected successfully';
          break;
        case 'activate':
          endpoint = `/food-providers/${providerId}/activate`;
          message = 'Food provider activated successfully';
          break;
        case 'deactivate':
          endpoint = `/food-providers/${providerId}/deactivate`;
      ...
      if (response.data && response.status === 200) {
        ...
        throw new Error(`Failed to ${action} food provider`);
      }
    } catch (error) {
      console.error(`Error ${action}ing food provider:`, error);
      Alert.alert('Error', `Failed to ${action} food provider. Please try again.`);
    }
  };
      ...
      <View style={styles.providerCardHeader}>
        ...
        <View style={styles.providerMainInfo}>
          ...
          <Text style={styles.providerLocation} numberOfLines={1}>
            <Ionicons name="location" size={12} color="#64748b" /> {item.location}
          </Text>
          ...
          <View style={styles.modalContainer}>
            ...
            <ScrollView style={styles.modalContent}>
              ...
              <View style={styles.modalSection}>
                ...
                <Text style={styles.modalText}>{selectedProvider.location}</Text>
              </View>
              ...
              <View style={styles.modalSection}>
                ...
                <Text style={styles.modalText}>Name: {selectedProvider.owner.name}</Text>
                <Text style={styles.modalText}>Email: {selectedProvider.owner.email}</Text>
              </View>
