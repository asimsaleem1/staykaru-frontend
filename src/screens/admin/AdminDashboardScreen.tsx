...
interface Booking {
  _id: string;
  accommodation: { title: string; price: number };
  student: { name: string };
  checkIn: string;
  checkOut: string;
  totalAmount: number;
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      console.log('🔄 Fetching admin dashboard data from backend...');
        const [usersResponse, accommodationsResponse, foodProvidersResponse, bookingsResponse] = await Promise.all([
        api.get('/users'),
        api.get('/accommodations'),
        api.get('/admin/food-providers').catch(() => ({ data: [] })), // Fallback if endpoint doesn't exist
        api.get('/admin/bookings').catch(() => ({ data: [] })) // Fallback if endpoint doesn't exist
      ]);

      const allUsers: User[] = usersResponse.data || [];
      const allAccommodations: Accommodation[] = accommodationsResponse.data || [];
      const allFoodProviders: FoodProvider[] = foodProvidersResponse.data || [];
      const allBookings: Booking[] = bookingsResponse.data || [];

      console.log(`✅ Loaded ${allUsers.length} users, ${allAccommodations.length} accommodations, ${allFoodProviders.length} food providers, ${allBookings.length} bookings`);
      ...
      // Calculate revenue in Pakistani Rupees (PKR)
      ...
      const stats: DashboardStats = {
        ...
        foodProviders: allFoodProviders,
        bookings: allBookings
      };

      console.log('📊 Dashboard Stats with PKR Revenue:', {
        totalRevenue: `PKR ${stats.totalRevenue.toLocaleString()}`,
        monthlyRevenue: `PKR ${stats.monthlyRevenue.toLocaleString()}`,
        totalBookings: stats.totalBookings,
        totalFoodProviders: stats.totalFoodProviders
      });
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Revenue Section */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>💰 Revenue (Pakistani Rupees)</Text>
          <View style={styles.statsGrid}>
            <StatCard 
              title="Total Revenue" 
            ...
            <StatCard 
              ...
              value={`PKR ${dashboardStats.monthlyRevenue.toLocaleString()}`}
